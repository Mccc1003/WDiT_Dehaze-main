...
import utils
from models.Net import DiT
from models.wavelet import DWT, IWT
from pytorch_msssim import ssim
from models.mods import HFB
from models.MPB import Dehaze
...
class Net(nn.Module):
    def __init__(self, args, config):
        ...
        self.high_enhance0 = HFB()
        self.Unet = DiT(config)
        self.dehaze_block1 = Dehaze()
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            enhanced_pred_x = self.dehaze_block1(pred_x)

            data_dict["input_high0"] = input_high0
            data_dict["gt_high0"] = gt_high0
            data_dict["pred_LL"] = denoise_LL
            data_dict["gt_LL"] = gt_LL
            data_dict["noise_output"] = noise_output
            data_dict["pred_x"] = enhanced_pred_x
            data_dict["e"] = e
...
class DenoisingDiffusion(object):
    ...
    def train(self, DATASET):
        ...
        for epoch in range(self.start_epoch, self.config.training.n_epochs):
            ...
            for i, (x, y) in enumerate(train_loader):
                ...
                low_loss, high_loss, photo_loss = self.estimation_loss(x, output)

                loss = 2*low_loss + high_loss + photo_loss
                if self.step % 10 == 0:
                    print("step:{}, lr:{:.6f}, low_loss:{:.4f}, high_loss:{:.4f}, photo_loss:{:.4f}, loss:{:.4f}"
                            .format(self.step, self.scheduler.get_last_lr()[0],
                                                            low_loss.item(), high_loss.item(), photo_loss.item(), loss.item()
                                                            ))
    ...
    def estimation_loss(self, x, output):
        ...
        # =============low loss==================
        low_loss = self.l2_loss(noise_output, e) + (0.1 * self.l2_loss(pred_LL, gt_LL))

        # =============high loss==================
        high_loss = self.l1_loss(input_high0, gt_high0)
        ...
        return low_loss, high_loss, photo_loss
